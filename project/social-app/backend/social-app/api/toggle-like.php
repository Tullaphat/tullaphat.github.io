...
try {
    ...
    if ($existing) {
        $deleteSql = "DELETE FROM {$postLikesTable} WHERE id = :id LIMIT 1";
    ...
    $countSql = "SELECT COUNT(*) AS total FROM {$postLikesTable} WHERE post_id = :post_id";
    $countStmt = $pdo->prepare($countSql);
    $countStmt->execute(['post_id' => $postId]);
    $likeCount = (int)($countStmt->fetch()['total'] ?? 0);
    ...
    json_response(500, [
        'success' => false,
        'message' => 'Database error while toggling like.',
