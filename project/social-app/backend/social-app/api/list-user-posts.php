...
$profileUsername = strtolower(trim((string)($input['profileUsername'] ?? '')));
...
if ($profileUsername === '') {
    json_response(422, [
        'success' => false,
        'message' => 'Valid profileUsername is required.',
    ]);
}
...
try {
    ...
    $profileSql = "
        SELECT id, username, first_name, last_name, profile_image_filename
        FROM {$usersTable}
        WHERE username = :username
           OR CONCAT('user', id) = :username_fallback
        LIMIT 1
    ";

    $profileStmt = $pdo->prepare($profileSql);
    $profileStmt->bindValue(':username', $profileUsername, PDO::PARAM_STR);
    $profileStmt->bindValue(':username_fallback', $profileUsername, PDO::PARAM_STR);
    $profileStmt->execute();
    ...
    $profile = [
        ...
        'username' => (string)($profileRow['username'] ?? fallback_username_from_id((int)$profileRow['id'])),
        'firstName' => (string)$profileRow['first_name'],
        'lastName' => (string)$profileRow['last_name'],
        'profileImageFilename' => (string)($profileRow['profile_image_filename'] ?? ''),
        'profileImageUrl' => profile_image_url($profileRow['profile_image_filename'] ?? null),
    ];

    $profileUserId = (int)$profileRow['id'];
    ...
    $postsStmt->bindValue(':profile_user_id', $profileUserId, PDO::PARAM_INT);
    ...
    json_response(500, [
        'success' => false,
        'message' => 'Database error while loading profile posts.',
