  ...
  methods: {
    ...
    async deletePhotoFromServer(photoUrl) {
      ...
      const response = await fetch(`${apiBase}/delete-photo.php`, {
        ...
        body: JSON.stringify({
          photoUrl: this.normalizePhotoSrc(photoUrl),
    ...
    async uploadPhotoToServer(blob, filename) {
      ...
      return this.normalizePhotoSrc(result.photoData || result.filename || '');
    },
    ...
    getPhotoSrc(collection, rarity, slotIndex, personIndex = null) {
      ...
      if (!data) return '';
      if (typeof data === 'string') return this.normalizePhotoSrc(data);
    ...
    normalizePhotoSrc(rawValue) {
      ...
      if (src.startsWith('http://') || src.startsWith('https://')) {
        try {
          ...
          if (parsed.pathname.includes('/api/uploads/')) {
            return `${apiBase}/uploads/${parsed.pathname.split('/').pop()}`;
          }
      ...
      if (src.includes('/')) {
        const normalized = fromUploadsPath(src);
